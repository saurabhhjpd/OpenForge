...
function display_ml_details($group_id, $list_server, $result, $i) {
    ...
    echo '<div class="new_line"><i class="tuleap-services-angle-double-right tuleap-services-mail"></i>&nbsp;<b>'.db_result($result, $i, 'list_name').'</b> [';
    ...
    echo '<A HREF="?group_id='. $group_id .'&amp;action=admin&amp;id='. db_result($result, $i, 'group_list_id') .'">List Admin</A>';		
	echo ' | <A HREF="?group_id='. $group_id .'&amp;action=listinfo&amp;id='. db_result($result, $i, 'group_list_id') .'">Preferences</A>  | ';
	
	//List Admin | Preferences | Archives 
    ...
    if($html_a) {
        ...
        if ($list_is_public) {
            ...
            echo ' <A HREF="?group_id='. $group_id .'&amp;action=pipermail&amp;id='. db_result($result, $i, 'group_list_id') .'">Archives</A>';
        } else {
    ...
    echo ' ]<br>'.  db_result($result, $i, 'description') .'<p></div>';
}
    ...
    if ($request->exist('action')) {
        if ($request->exist('id')) {
            ...
            if (db_numrows($result)) {
                ...
                echo '<a href="?group_id='. $group_id .'">Go Back</a>';
		...
		// (Mailing lists in OpenForge are provided via GNU Mailman)
        //echo '<P>(Mailing lists in OpenForge are provided via <a href="http://www.list.org">GNU Mailman</a>) ';
