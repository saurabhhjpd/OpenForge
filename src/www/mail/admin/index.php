...
if ($group_id && user_ismember($group_id, 'A')) {
    ...
    if ($request->existAndNonEmpty('add_list')) {
		...
		echo '<p>Lists are named in this manner: <em>projectname-listname@openforge.gov.in</em></p><BR>';
        ...
        echo '   <BR> <P>
            ...
            <BR><B>' . $Language->getText('mail_admin_index', 'desc') . ':</B><BR>
        ...
        if (!$result || $rows < 1) {
            ...
            echo '
            <H3>' . $Language->getText('mail_admin_index', 'update_mail_list') . '</H3>
            <P>
            ' . $Language->getText('mail_admin_index', 'admin_lists_here') . 'OpenForge. <P><br>';
