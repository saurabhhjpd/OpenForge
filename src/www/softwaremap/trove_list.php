...
echo'
	<h2>Browse all OpenForge Projects</h2>
	<HR NoShade>
';
...
for ($i=0;$i<$folders_len;$i++) {
	...
	if ($folders_ids[$i] != $form_cat) {
            if (!isset($discrim_url)) $discrim_url="";
			print '<div class="span12">';
            print '<div class="header_bread span12"><A href="/softwaremap/trove_list.php?form_cat='
...
while ($row_sub = db_fetch_array($res_sub)) {
	for ($sp=0;$sp<($folders_len*2);$sp++) {
        ...
        if (!isset($discrim_url)) $discrim_url="";
	print ('<div class="span4 subproj"><a href="trove_list.php?form_cat='.$row_sub['trove_cat_id'].$discrim_url.'"><span class="full_click">');
	//html_image("ic/cfolder15.png",array());
        $nb_proj_in_cat=($row_sub['subprojects']?$purifier->purify($row_sub['subprojects']):'0');
        $nb_listed_projects+=$nb_proj_in_cat;
	print (''.$purifier->purify($row_sub['fullname']).'</span></a> </br><I>('
		.$nb_proj_in_cat
		.' '.$Language->getText('softwaremap_trove_list','projs').')</I></div>'); //<BR>
}
...
AND trove_cat_root = ". $form_cat;
    ...
    //print "&nbsp; ";
    print '<div class="span4 subproj">';
    //html_image("ic/cfolder15.png",array());

    print '<a href="/softwaremap/trove_list.php?form_cat='.$form_cat.'&special_cat=none"><em>'.$Language->getText('softwaremap_trove_list','not_categorized').'</em></a> <I>('.$nb_not_cat.' '.$Language->getText('softwaremap_trove_list','projs').')</I>';
...
// print $html_limit."</br><HR>\n";
if ($querytotalcount ==0) {
	$html_limit = '<div class="span12"><SPAN><FONT size="-1">';
	print $html_limit."</br>\n";
    //$html_limit .= $Language->getText('softwaremap_trove_list','no_project_in_cat')."<br>\n";
}
else {
$html_limit = '<div class="span4"><SPAN><FONT size="-1">';
...
for ($i_proj=1;$i_proj<=$querytotalcount;$i_proj++) { 
	...
	if ($row_grp && $viewthisrow) {
		...
		$originalDate = format_date($GLOBALS['Language']->getText('system', 'datefmt'),$row_grp['register_time']);
		$newDate = date("d-m-Y", strtotime($originalDate));
		print '<BR>'.$Language->getText('softwaremap_trove_list','register_date').' <B>'.$newDate.'</B>';
