...
// project_admin_header(array('title'=>$Language->getText('project_admin_index','p_admin',$group->getPublicName()),'group'=>$group_id,
			   ...
			   project_admin_header(array('title'=>$Language->getText('project_admin_index','p_admin',$group->getPublicName())));

/*
	Show top box listing trove and other info
*/

echo '<TABLE width=100% border=0 class="trv_list">
<TR valign=top><TD width=50%>';
//$HTML->box1_top('<span class="title_bold">Project Info&nbsp;:&nbsp;</span>' . $group->getPublicName()); 
...
'.'<span class="title_bold">Short Description&nbsp;:&nbsp;</span>' . $hp->purify($group->getDescription(), CODENDI_PURIFIER_LIGHT);
...
if ($GLOBALS['sys_use_trove'] != 0) {
    print ' 
	<div class="higlight"><P>
<B>'.'<span class="title_bold"><img border="0" alt="add" src="/themes/FlamingParrot/images/ic/add.png"> &nbsp;Enter Project Categorization&nbsp:&nbsp;</span>'. '<A href="/project/admin/group_trove.php?group_id='.$group_id.'">'
.'<B><span class="edit_trov_but">(edit)</span></B></A>'.
'
...
<UL>';
    ...
    while ($row_trovecat = db_fetch_array($res_trovecat)) {
			 array_push($trove_array,  explode( '::', $row_trovecat['fullpath']));
			 
			 $temp =explode( '::', $row_trovecat['fullpath']);
			 $trove_array_merge11[$temp[0]][] = $temp[1];
    }
	foreach($trove_array_merge11 as $key => $value){
		$feeds = $value[0].','. $value[1].','. $value[2].','. $value[3];
		print ('<LI><span class="major_head">'.$key.' : </span> <span class="minor_head">' . rtrim($feeds,',').'</span>'		  );
	}
    print '
</UL>
</div>
...
//$HTML->box1_top($Language->getText('project_admin_index','s_admin').'&nbsp;');
