...
class WikiServiceAdminViews extends WikiViews {
  ...
  function _displayEntryForm($act='', $id='', $name='', $page='', $desc='', $rank='') {
    ...
    print '<BR><form name="wikiEntry" method="post" action="'.$this->wikiAdminLink.'&view=wikiDocuments">
  ...
  function main() {
	...
	// case 'fr_FR': printf("<h2>Wiki %s - Administration</h2><h3><a href=%s&view=wikiDocuments>Gérer les documents Wiki</a></h3><p>Créer, supprimer, modifier et donner des permissions sur des documents Wiki.</p>", $this->wikiname, $this->wikiAdminLink);
		     ...
		     // break;
	case 'en_US':	     
	default :     //printf("<h2>Wiki  %s - Administration</h2><hr size='1' noshade=''><h3><a href= %s&view=wikiDocuments>Manage Wiki Documents</a></h3><p>Create, delete, modify and set specific permissions on Wiki Documents.</p>", $this->wikiname, $this->wikiAdminLink);
	default :     printf("<h2>Wiki  %s - Administration</h2><hr size='1' noshade=''>", $this->wikiname, $this->wikiAdminLink);
		     printf("<h3><a href=%s&view=wikiPages>Manage Wiki Pages</a></h3>", $this->wikiAdminLink); 	
		     printf("<BR><h3><a href=%s&view=wikiAttachments>Manage Wiki Attachments</a></h3>", $this->wikiAdminLink);	
			 printf("<BR><h3><a href=%s&view=wikiPerms>Set Wiki Permissions</a></h3>", $this->wikiAdminLink, $this->wikiname);
		     printf("<BR><h3><a href=%s&pagename=PhpWikiAdministration>PhpWiki Administration</a></h3>", $this->wikiLink);
		     break;
    // <p>Browse and set permissions on ressources attached on the Wiki.</p>
	// <p>Browse and set specific permissions on Wiki Pages.</p>
	// <p>Administration panel of the wiki engine. This propose a set of tools to delete and rename pages.</p>
	// <p>Set permissions on whole %s Wiki.</p>
    }
    ...
    */
    function _browsePages(&$pageList) {
        ...
        foreach($pageList as $pagename) {
            ...
            print '<td style="width: 50%;"><a href="'.$this->wikiLink.'&pagename='.urlencode($pagename).'">'.$purifier->purify($pagename).'</a></td>';
