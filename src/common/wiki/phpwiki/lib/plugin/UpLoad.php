...
ws[cfh]");
		...
		$fullstring = $request->getPostURL();
		$parsed = get_string_between($fullstring, '&', '&');
		
		// echo 'admin/index.php?'.$parsed.'&view=wikiAttachments'; // (result = dog)
		
		
		
			//print $request->getPostURL();
        $message = HTML();
	    ...
	    $err_header = HTML::h2(fmt("ERROR uploading '%s': ", $userfile_name));
            ...
            if($rev >= 0) {
                ...
                ?>				
				<script>
					opener.location.href = '<?php echo 'admin/index.php?'.$parsed.'&view=wikiAttachments';?>';
					//setTimeout(function(){ alert("File successfully uploaded"); }, 5000);
					window.close();
       ...
       // $attchList->pushContent(HTML::a(array('href' => $url.'&offset='.($offset+$limit)),
		...
		// https://10.22.119.29/wiki/group_id=101
		// $url_use = '/admin/index.php?'.$parsed.'&view=wikiAttachments';
        $result = HTML();
        $result->pushContent($form);
		// https://10.22.119.29/wiki/admin/index.php?group_id=101&view=wikiAttachments
		// https://10.22.119.29/wiki/index.php?pagename=UpLoad&group_id=101&pv=1
		// $result->pushContent(HTML::a(array('href' => $url_use),
